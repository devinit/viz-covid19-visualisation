<template>
  <b-card bg-variant="light" style="margin-bottom: 20px;">
    <b-btn
      :href="`http://d-portal.org/q.html?aid=${activity['iati-identifier']._text}`"
      variant="secondary"
      class="float-right"
      size="sm">
      View on D-Portal
    ...
    <h3>
      {{ activity.title.narrative._text }} (Total {{ transactionTypeName }}:
      {{ activity._attributes['default-currency'] }}
      {{ getTotalExpenditure(activity).toLocaleString(undefined, {
